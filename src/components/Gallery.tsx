"use client";
import { motion } from "motion/react";
import { cn } from "@/lib/utils";

export const Gallery = () => {
  interface Gallery {
    img: string;
    className?: string;
  }
  const Gallery: Gallery[] = [
    {
      img: "/bluesky.jpeg",
    },
    {
      img: "/bridge.jpeg",
      className: "",
    },
    {
      img: "/lights.jpeg",
    },
    {
      img: "/sunset.jpeg",
    },
    {
      img: "/street.jpeg",
    },
  ];

  return (
    <div className="flex h-screen w-full flex-col items-center justify-center bg-linear-to-b from-zinc-50 from-20% to-pink-300/50">
      <h2 className="mt-15 ml-2 flex bg-linear-to-b from-neutral-700 to-neutral-900/80 bg-clip-text pb-2 text-3xl font-medium tracking-tight text-transparent md:mt-20 md:text-[37px]">
        Look at some nice photo collection!
        <svg
          width="72"
          height="72"
          viewBox="0 0 72 72"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          className="size-9 pb-1 pl-2 md:size-10.5"
        >
          <rect width="72" height="72" fill="url(#pattern0_22_1779)" />
          <defs>
            <pattern
              id="pattern0_22_1779"
              patternContentUnits="objectBoundingBox"
              width="1"
              height="1"
            >
              <use xlinkHref="#image0_22_1779" transform="scale(0.0138889)" />
            </pattern>
            <image
              id="image0_22_1779"
              width="72"
              height="72"
              preserveAspectRatio="none"
              xlinkHref="data:image/png;base64,"
            />
          </defs>
        </svg>
      </h2>
      {/* <p className="pb-4 text-[12px] text-neutral-600 md:text-[16px]"></p> */}
      <div className="mobile-L:max-w-sm mt-5 flex max-w-80 gap-1.5 md:max-w-xl md:gap-3 lg:max-w-full">
        {Gallery.map((data, idx) => {
          return (
            <div className="" key={idx}>
              <motion.img
                key={idx}
                initial={{
                  width: 130,
                  objectFit: "cover",
                }}
                whileHover={{
                  width: 250,
                }}
                whileTap={{
                  width: 250,
                }}
                transition={{
                  type: "spring",
                  mass: 0.5,
                  stiffness: 100,
                  damping: 9,
                }}
                alt=""
                src={data.img}
                className={cn(
                  "h-60 rounded-sm shadow-lg md:h-70 lg:h-100",
                  data.className,
                )}
                width={0}
                height={0}
              ></motion.img>
            </div>
          );
        })}
      </div>
      <h3 className="relative top-7 text-sm text-black lg:top-10">
        crafted by @Rajveer
      </h3>
    </div>
  );
};
